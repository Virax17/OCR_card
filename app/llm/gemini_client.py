...
def structure_ocr_text(event_id: str, front_text: str, back_text: str | None = None) -> dict:
    prompt = f"""
Extract business card contact details from OCR text.
...
- company and business mean the top-most brand/logo/company name on the card. Prefer the stylized name printed at the top of the front side over address lines, buildings, legal suffix lines, or back-side branch names.
- If the logo/company name is stylized, read it visually as text and return the brand/company name, not the design description.
- country_code is the international dialing code, for example +91, +1, +971.
...
""".strip()
    ...
    if not snapshot.allowed or snapshot.daily_tokens + prompt_tokens > snapshot.daily_token_limit:
        record_usage(
            event_id,
            model=GEMINI_MODEL,
            purpose="business_card_structuring",
